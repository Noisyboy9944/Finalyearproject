...
import { ChatCircleDots, X, PaperPlaneRight, Robot, User, SpinnerGap } from '@phosphor-icons/react';
...
const ChatBot = () => {
    ...
    const [messages, setMessages] = useState([
        { role: 'assistant', content: "Hi! I'm your UniLearnHub AI assistant. I can help you navigate courses, explain concepts, or answer any learning questions. What can I help you with?" }
    ]);
    ...
    const renderContent = (text) => {
        ...
        html = html.replace(/```(\w*)\n?([\s\S]*?)```/g, '<pre class="bg-gray-800 text-green-300 rounded-lg p-3 my-2 text-xs overflow-x-auto font-mono">$2</pre>');
        // Convert inline code
        html = html.replace(/`([^`]+)`/g, '<code class="bg-gray-200 text-gray-800 px-1 py-0.5 rounded text-xs font-mono">$1</code>');
            ...
            <AnimatePresence>
                {!isOpen && (
                    <motion.button
                        ...
                        className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-gradient-to-br from-indigo-600 to-purple-600 text-white shadow-lg shadow-indigo-500/30 flex items-center justify-center hover:shadow-xl transition-shadow"
                    >
                                ...
                                >
                                    ...
                                    <div className="bg-white border border-gray-100 shadow-sm rounded-2xl rounded-bl-md px-4 py-3">
                                        <div className="flex items-center gap-1.5">
                                            <SpinnerGap size={16} className="animate-spin text-purple-500" />
                            ...
                            <div className="px-4 py-2 bg-white border-t border-gray-100 flex gap-2 overflow-x-auto shrink-0">
                                {quickQuestions.map((q, i) => (
                                    <button
                                        ...
                                        className="text-xs bg-indigo-50 text-indigo-600 px-3 py-1.5 rounded-full whitespace-nowrap hover:bg-indigo-100 transition-colors border border-indigo-100"
                                    >
                                        {q}
                                    </button>
                                ))}
